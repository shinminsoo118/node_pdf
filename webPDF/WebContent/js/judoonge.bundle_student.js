    ...
    self.Jabber = {
        ...
        Room: {
            ...
            Leave: function (roomJid) {
                ...
                // hwcho
                Judoonge.Core.getConnection().muc.leave(roomJid,
                    Judoonge.Core.getRoom(roomJid).getUser().getNick(),
                    function () {
                    });
    ...
    self.Jabber = {
        ...
        Presence: function (msg) {
            ...
            if (msg.attr('id') == Strophe.ID.CREATE_ROOM) {
                ...
                var statusCode = msg.children('x').children('status').attr('code');
                var roomJid = roomName + "@" + Judoonge.serverInfo.conferenDomain;
                ...
                if (msg.children('x[xmlns^="' + Strophe.NS.MUC + '"]').length > 0) {
                    ...
                    var fromUsrName = Strophe.escapeNode(Strophe.getNodeFromJid(fromUsr));
                    if (!msg.attr("type")) {
                        ...
                        Judoonge.Core.Action.Jabber.presenceContactResponse(
								fromUsrJid, Strophe.DEF.PRES_TYPE_SUBSCRIBED); // 정보공유응답(승인으로 보냄)
                        ...
                        Judoonge.Core.log('[Judoonge.Core.Event.Jabber] Presence etc.. ');
                    }
                }
            }
            return true;
        ...
        Message: function (msg) {
            ...
            if (procID == Strophe.ID.BUDDY_JOINCALL) {
                ...
                // null, null, null);
                teacherId = Strophe.getNodeFromJid(msg.attr('from'));

                console.log("선생선생선생 => ", teacherId);
                $("#studentList-" + teacherId).attr("teacher", "teacher");
            ...
            if (args.message.name) {
                ...
                if (Strophe.getNodeFromJid(Judoonge.Core.getUser().getJid()) == args.message.name) {
                    ...
                    $("#mainMenuList1-" + tmpId + " li a").removeClass(
							"chatMenuSelect");
                    $("#mainMenuList1-" + tmpId + " li a[href='#voiceBox-" + tmpId + "']")
    ...
    self.mainList = {
        ...
        update: function (obj, args) {
            ...
            if (args.type == "result") {
                    ...
                    for (var index in Judoonge.DEF.loginlist) {
                        if (Judoonge.DEF.loginlist[index] == args.jid) {
                            if (Strophe.getNodeFromJid(args.jid) != Strophe.DEF.FACEBOOK_FRIEND) {
                                $("#friendimg-" + Strophe.getNodeFromJid(args.jid)).attr("src", "images/main/onlineImg.png");
                            }
                ...
                $("#studentList-" + Strophe.getNodeFromJid(args.jid)).attr("status", "on");
                ...
                // logout process
                Judoonge.Core.log("Judoonge.View.Observer.mainList logout -> jid/res : " + args.jid);
        ...
        update: function (obj, args) {
            ...
            if (args.type == "result") {
                if (args.id == Strophe.ID.MUC_ROOMS_GET) {
                    ...
                    if (args.userNick == Strophe.getNodeFromJid(Judoonge.Core.getUser().getJid())) {
                        Judoonge.Core.Action.Jabber.Room.Leave(args.roomJid);
                    }
                } else {
                    // etc id
                    Judoonge.Core
...
function onLoad() {
    ...
    webrtc.startApp({
        ...
        incomingCall: function (farParty, roomJid) {
            ...
            // hwcho 2014-05-30 accept 버튼 표시
            $("#accept").show();
            // startRinging();
        },

        terminatedCall: function () {
            if ($("#studentList-" + teacherId).attr("status") == "on") {
                $("#studentList-" + teacherId + " a img").attr("src", "image/student_on_image.png");
            } else if ($("#studentList-" + teacherId).attr("status") == "off") {
                $("#studentList-" + teacherId + " a img").attr("src", "image/student_off_image.png");
            }
